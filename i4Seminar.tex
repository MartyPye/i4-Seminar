...
The common ones can sense movement, pressure, light, sound, electrical potential, temperature, moisture, location and heart rate. 
It is common for one device to incorporate several sensing elements. 
But just gathering the data is not enough. 
It needs to be processed and transmitted. 
The data processing can be done locally, on the device itself, or after the data has been transmitted. 
For the transmission, a variety of communication protocols can be used like Wi-Fi, Bluetooth, ZigBee, 3G, etc. A recent version of Bluetooth named Bluetooth low-energy allows mobile devices to send data with more battery efficiency. 
Once this data has been processed and transmitted, it needs to be converted into some sort of human-readable form. The usual way of making the information accessible is through graphical representation. 
There are web interfaces and mobile applications which display beautifully designed charts and statistics of the data, in an intuitively understandable manner. 
...
Not all information gathered by sensors is aimed at being visualized and displayed to the user in some way. 
...
The IoT-trend is making more and more objects context sensitive by sensing one or many of the above features.

Once smart objects have gathered some specific information information, the user needs some means of interacting with the object. 
The different possibilities of interaction, together with their pros and cons are discussed in the following chapter.
...
Ideally, this interaction should be seamlessly and ubiquitously integrated into everyday artefacts, and it should allow input as well as output. 
...
\subsubsection{Embedded devices vs.\ interaction devices}
Embedding the interaction device into existing objects means integrating interaction opportunities into existing artefacts, whereas interaction devices are connected with the object of interest when interaction is needed.
This has the advantage that the hardware necessary for the interaction can be removed when not needed.
...
However, these embedded interaction features can be subject to another challenge, the invisibility dilemma.

\subsubsection{Invisibility dilemma}
When augmenting an everyday object, it is important to preserve the original function, look and feel.
...
Kranz et al. investigated several particular areas and their characteristic interaction methods \cite{kranz10} which are discussed in the following chapter.
